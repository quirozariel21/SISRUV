...
public class ILogDaoImpl implements ILogDao{
    ...
    public int insert(int num, ILog log) {
        ...
        try {
            if(num == 1){
                CallableStatement l = connection.prepareCall("select sp_operaciones_indicador_log(?,?,?,?,?,?,?)");
    ...
    public int max_event() {
        ...
        Connection connection = conn.getConnectionDB();
            ...
            String sql = "select max(id) maxp from log_event_indicador";
    ...
    public int max_error() {
        ...
        Connection connection = conn.getConnectionDB();
            Statement statement=connection.createStatement();
            String sql = "select max(id) maxp from log_error_indicador";
            ResultSet resultSet = statement.executeQuery(sql);
            if(resultSet.next()){
                max = resultSet.getInt("maxp");
            }
